...
import redisQueue from "../lib/redis-queue"; // default export with emailSendQueue, followUpQueue, etc.
 ...
 */
async function safeGetCounts(queue: any) {
  try {
    ...
    console.warn(
      "[queue.routes] getJobCounts failed for queue:",
      queue?.name,
      err
    );
    return {
...
async function getQueueStats() {
  const emailQ = redisQueue.emailSendQueue;
  const automationQ = redisQueue.followUpQueue;
  ...
  return {
    ...
    meta: {
      ...
      prefix: (emailQ && (emailQ.opts as any)?.prefix) || undefined,
    },
  };
}
...
router.get("/health", async (_req, res) => {
  try {
    ...
    const pendingAutomation =
      (stats.automation.waiting || 0) + (stats.automation.active || 0);

    // thresholds (tune as needed)
    const isHealthy =
      connectionStatus && pendingEmail < 1000 && pendingAutomation < 500;
...
router.get("/metrics", async (_req, res) => {
  try {
    ...
    const automationProcessed =
      (stats.automation.completed || 0) + (stats.automation.failed || 0);
    ...
    const metrics = {
      performance: {
        ...
        automationQueue: {
          ...
          backlog:
            (stats.automation.waiting || 0) + (stats.automation.delayed || 0),
      ...
      recommendations: {
        email:
          (stats.email.waiting || 0) > 50
            ? "Consider scaling email workers"
            : "Optimal",
        automation:
          (stats.automation.waiting || 0) > 20
